...
type
  ...
  TBudgets = class(TWorkForm)
    ...
    procedure bInsertLine7Click(Sender: TObject);
    procedure bInsertLine8Click(Sender: TObject);
    ...
    procedure NiceGrid1BeforeShowEdit(Sender: TObject; ARec: TRect;
...
begin
  ...
  begin
    if not aAll then
      ...
      begin
        Inc(aRow);
        if aRow >= (NiceGrid1.RowCount) then
          break;
        continue;
      end;
    ...
    begin
      tmp := StrToFloatDef(NiceGrid1.CellsT['cTotalAmount', aRow],
        0, DefaultFormatSettings);
      if aTime < tmp then
      begin
        aTime := tmp;
...
begin
  ...
  if (NiceGrid1.CellsT['cType', aRow] = 'UO') or
    (NiceGrid1.Rows[aRow].NodeIndex > 0) then
  begin
...
function TBudgets.InsertarItem(Dato: ansistring; idx, ARow: integer): integer;
...
begin
    ...
    try
      ...
      begin
        ...
        df := InsertarItem(zqElemCom.FieldByName('ELEMENT_CODE').Text,
          idx + 1, ARow + i);
        NiceGrid1.CellsT['cUnitAmount', ARow + i] :=
  ...
  if Nicegrid1.Rows[ARow].NodeIndex > 0 then Exit;

  i := ARow - 1;
  ind := '';
  while i >= 0 do
  begin
    if (Nicegrid1.Rows[i].NodeIndex > 0) or (NiceGrid1.CellsT['cType', i] = '') or
      (NiceGrid1.CellsT['cType', i] = 'ST') then
    begin
      Dec(i);
      continue;
    end;
    ind := NiceGrid1.CellsT['cIndex', i];
    Break;
  end;

  if ind <> '' then ind := IncStr(ind, 1)
  else ind := '1';

  NiceGrid1.CellsT['cIndex', ARow] := ind;
      ...
      begin
        ...
        begin
          ...
          Res.ResourceQuantity :=
            StrToFloatDef(NiceGrid1.CellsT['cUnitAmount', aRow], 0);
          case NiceGrid1.CellsT['cType', aRow] of
            'MO': Res.ResourceType := rtLabor;
            'MA', 'DI': Res.ResourceType := rtMaterial;
            'HE': Res.ResourceType := rtTool;
...
procedure TBudgets.bInsertLine7Click(Sender: TObject);
begin
  NiceGrid1.Font.Size := NiceGrid1.Font.Size + 1;
end;

procedure TBudgets.bInsertLine8Click(Sender: TObject);
begin
  NiceGrid1.Font.Size := NiceGrid1.Font.Size - 1;
end;
...
begin
  ...
  begin
    if NiceGrid2.CellsT['cCheck', i] <> '1' then
      continue;
...
begin
  ...
  begin
    ...
    begin
      ...
      zqDocVentaData.FieldByName('ELEMENT_DESCRIPTION').AsString :=
        NiceGrid1.CellsT['cDescription', i];
      zqDocVentaData.FieldByName('ELEMENT_UNIT_AMOUNT').Text :=
        NiceGrid1.CellsT['cUnitAmount', i];
      zqDocVentaData.FieldByName('ELEMENT_TOTAL_AMOUNT').Text :=
        NiceGrid1.CellsT['cTotalAmount', i];
      zqDocVentaData.FieldByName('ELEMENT_UNIT').Text := NiceGrid1.CellsT['cUnit', i];
      zqDocVentaData.FieldByName('ELEMENT_PRICE').Text :=
        NiceGrid1.CellsT['cUnitPrice', i];
      ...
      zqDocVentaData.FieldByName('ELEMENT_INDEX').AsInteger :=
        NiceGrid1.Rows[i].NodeIndex;
      zqDocVentaData.Post;
      zqDocVentaData.Next;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      NiceGrid1.CellsT['cTitle', ARow] :=
        zqDocVentaData.FieldByName('ELEMENT_TITLE').Text;
      NiceGrid1.CellsT['cDescription', ARow] :=
        zqDocVentaData.FieldByName('ELEMENT_DESCRIPTION').AsString;
      NiceGrid1.CellsT['cUnitAmount', ARow] :=
        zqDocVentaData.FieldByName('ELEMENT_UNIT_AMOUNT').Text;
      NiceGrid1.CellsT['cTotalAmount', ARow] :=
        zqDocVentaData.FieldByName('ELEMENT_TOTAL_AMOUNT').Text;
      NiceGrid1.CellsT['cUnit', ARow] := zqDocVentaData.FieldByName('ELEMENT_UNIT').Text;
      NiceGrid1.CellsT['cUnitPrice', ARow] :=
        zqDocVentaData.FieldByName('ELEMENT_PRICE').Text;
    ...
    begin
      ...
      begin
        ...
        NiceGrid1.CellsT['cDescription', ARow] :=
          zqDocVentaData.FieldByName('ELEMENT_DESCRIPTION').AsString;
        NiceGrid1.CellsT['cUnitAmount', ARow] :=
          ZQ.FieldByName('ELEMENT_UNIT_AMOUNT').Text;
        NiceGrid1.CellsT['cTotalAmount', ARow] :=
          ZQ.FieldByName('ELEMENT_TOTAL_AMOUNT').Text;
...
begin
  ...
  if TControl(Sender).Tag > 0 then
    NiceGrid1.Rows[NiceGrid1.Row].ImageIndex := TControl(Sender).Tag;
...
begin
  ...
  if StringReplace(NiceGrid1.CellsT['cCode', NiceGrid1.Row], ' ', '',
    [rfReplaceAll]) = '' then
    exit;
...
begin
  ...
  NiceGrid1.Rows[NiceGrid1.Row].NodeIndex :=
    NiceGrid1.Rows[NiceGrid1.Row - 1].NodeIndex + 1;
  NiceGrid1.Row := NiceGrid1.Row - 1;
end;
...
begin
  ...
  if NiceGrid1.CellsT['cCode', Row] <> '' then
    Exit;
...
procedure TBudgets.NiceGrid1BeforeShowEdit(Sender: TObject; ARec: TRect;
  var aCanShow: boolean);
begin
  case NiceGrid1.Columns[NiceGrid1.Col].Name of
    'cIndex', 'cDiscount', 'cBenefit':
      if (NiceGrid1.Rows[NiceGrid1.Row].NodeIndex > 0) or
        (NiceGrid1.CellsT['cType', NiceGrid1.Row] = '') then
        aCanShow := False;
...
begin
  ...
  begin
    case NiceGrid1.Columns[ACol].Name of
      'cTitle':
      begin
        TInterval(NiceGrid1.Rows[ARow].ExternalObject).Task :=
          NiceGrid1.Cells[ACol, ARow];
      end;
      'cUnitAmount':
      begin
        //TInterval(NiceGrid1.Rows[ARow].ExternalObject).Task := NiceGrid1.Cells[ACol, ARow];
        //TInterval(NiceGrid1.Rows[ARow].ExternalObject).Amount := NiceGrid1.CellsT['cTotalAmount', ARow];
      end;
    end;
  end;
end;
  ...
  begin
    ...
    case NiceGrid1.Columns[ACol].Name of
      ...
      begin
        if (NiceGrid1.CellsT['cType', ARow] = '') and
          (NiceGrid1.CellsT['cTitle', ARow] <> '') then
    ...
    begin
      case NiceGrid1.Columns[ACol].Name of
        ...
        'cUnitAmount':
        begin
          TInterval(NiceGrid1.Rows[ARow].ExternalObject).FinishDate :=
...
begin
  ...
  {case NiceGrid1.Columns[ACol].Name of
    'cIndex', 'cDiscount', 'cBenefit':
      if (NiceGrid1.Rows[ARow].NodeIndex > 0) or
        (NiceGrid1.CellsT['cType', ARow] = '') then
        NiceGrid1.Columns.Items[ACol].ReadOnly := True;
    'cCode':
    begin
      //zqElemento.Open;
...
begin
  ...
  begin
    ...
    if CRow < (NiceGrid1.RowCount - 1) then
      offset := offset + 1 + InsertarItem(From.DataSource.DataSet.FieldByName(
        'CODE').Text, NiceGrid1.Rows[CRow + 1 + offset].NodeIndex, CRow + offset)
    else
      offset := offset + 1 + InsertarItem(from.DataSource.DataSet.FieldByName(
        'CODE').Text, NiceGrid1.Rows[CRow + offset].NodeIndex, CRow + offset);
    // insertar en gantt también:
    addTasks(cRow, FGantt.IntervalCount);
    //NiceGrid1.Rows[cRow].ExternalObject :=  FGantt.Interval[FGantt.IntervalCount - 1];
  end;
...
procedure TBudgets.NiceGrid1KeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
begin
  ...
  case pcAll.TabIndex of
    ...
    begin
      ...
      begin
        ...
        NiceGrid2.CellsT['cCode', NiceGrid2.RowCount - 1] :=
          NiceGrid1.CellsT['cCode', i];
        NiceGrid2.CellsT['cTitle', NiceGrid2.RowCount - 1] :=
          NiceGrid1.CellsT['cTitle', i];
        NiceGrid2.CellsT['cAmount', NiceGrid2.RowCount - 1] :=
          NiceGrid1.CellsT['cTotalAmount', i];
        NiceGrid2.CellsT['cUnit', NiceGrid2.RowCount - 1] :=
          NiceGrid1.CellsT['cUnit', i];
        NiceGrid2.CellsT['cBuy', NiceGrid2.RowCount - 1] :=
          NiceGrid1.CellsT['cTotalAmount', i];
      end;
    end;
    4: ;
