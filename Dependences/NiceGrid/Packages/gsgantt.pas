...
type
  {
  Tipos de conexiones:
    1. empiezan al mismo tiempo
    2. acaban al mismo tiempo
    3. empieza cuando acaba la anterior

  Implementar también el desfase de tiempo
  }
  TConnectionType = (ctNone, ctSameBeginning, ctSameEnd, ct1End_2Beginning);
...
type
  ...
  public
    ...
    property ResourceDuration: TDateTime read GetResourceDuration
      write SetResourceDuration;
  ...
  published
    property ShowTaskName: boolean read GetShowTaskName
      write SetShowTaskName default False;
  ...
  published
    ...
    property AllowDaysOff: boolean read GetAllowDaysOff
      write SetAllowDaysOff default True;
    ...
    property MajorScale: TTimeScale
      read GetMajorScale write SetMajorScale default tsWeek;
    ...
    property PixelsPerMinorScale: integer read GetPixelsPerMinorScale
      write SetPixelsPerMinorScale default 30;
    property PixelsPerLine: integer read GetPixelsPerLine
      write SetPixelsPerLine default 24;
    property StartDate: TDateTime read GetStartDate write SetStartDate;
    ...
    property OnStartDateChangd: TNotifyEvent
      read FStartDateChanged write FStartDateChanged;
  end;
...
begin
  Result := False;
  if (DayOfTheWeek(D) = 6) or (DayOfTheWeek(D) = 7) then
    Result := True;
end;
...
begin
  ...
  case TimeScale of
    ...
    begin
      ...
      S :=
        DateTimeToTimeStamp(IncTimeEx(TimeStampToDateTime(S),
        tsHour, Frac(IncAmount) * 24));
    end;
    tsWeek, tsWeekNum, tsWeekNumPlain:
    begin
      ...
      S :=
        DateTimeToTimeStamp(IncTimeEx(TimeStampToDateTime(S), tsDay,
        Frac(IncAmount) * 7));
    end;
    tsMonth:
    begin
...
begin
  ...
  case TimeScale of
    ...
    begin
      Result :=
        (FinishMonth + (FinishDay + FinishStamp.Time / 1000 / 60 /
        60 / WorkingHours) / MonthDays[IsLeapYear(FinishYear)]
        [FinishMonth]) - (StartMonth + (StartDay + StartStamp.Time /
        1000 / 60 / 60 / WorkingHours) / MonthDays[IsLeapYear(FinishYear)]
        [FinishMonth]) + (FinishYear - StartYear) * 12;
    end;
    tsQuarter:
    begin
      Result :=
        ((FinishMonth + (FinishDay + FinishStamp.Time / 1000 /
        60 / 60 / WorkingHours) / MonthDays[IsLeapYear(FinishYear)]
        [FinishMonth]) - (StartMonth + (StartDay + StartStamp.Time /
        1000 / 60 / 60 / WorkingHours) / MonthDays[IsLeapYear(FinishYear)]
        [FinishMonth])) / 3 + (FinishYear - StartYear) * 3;
    end;
    tsHalfYear:
    begin
      Result :=
        ...
        [FinishMonth]) - (StartMonth + (StartDay + StartStamp.Time /
        1000 / 60 / 60 / WorkingHours) / MonthDays[IsLeapYear(FinishYear)]
        [FinishMonth])) / 6 + (FinishYear - StartYear) * 6;
    end;
    tsYear:
    begin
      Result :=
        ...
        [FinishMonth]) - (StartMonth + (StartDay + StartStamp.Time /
        1000 / 60 / 60 / WorkingHours) / MonthDays[IsLeapYear(FinishYear)]
        [FinishMonth]) / 12) + (FinishYear - StartYear);
    end;
    else
    begin
      Result :=
        FinishStamp.Time / 1000 + FinishStamp.Date * WorkingHours *
        60 * 60 - StartStamp.Time / 1000 + StartStamp.Date * WorkingHours * 60 * 60;
    end;
  end;
end;
...
begin
  if FResourceDuration <> Value then
    FResourceDuration := Value;
end;

procedure TResource.SetResourceQuantity(Value: double);
begin
  if FResourceQuantity <> Value then
    FResourceQuantity := Value;
end;

procedure TResource.SetResourceCost(Value: double);
begin
  if FResourceCost <> Value then
    FResourceCost := Value;
end;
...
begin
  Result := DateUtils.CompareDate(TInterval(Item1).StartDate,
    TInterval(Item2).StartDate);
end;
...
begin
  Result := False;
  if self.FIntervals.Count > 0 then
    Result := True;
end;
...
begin
  if not Assigned(FGantt) then
    exit;
  FDrawRect.Left :=
    Round(UnitsBetweenDates(FGantt.Calendar.VisibleStart, StartDate,
    FGantt.Calendar.MinorScale, FHoursPerDay) * FGantt.Calendar.PixelsPerMinorScale);

  FDrawRect.Right :=
    Round(UnitsBetweenDates(FGantt.Calendar.VisibleStart, FinishDate,
    FGantt.Calendar.MinorScale, FHoursPerDay) * FGantt.Calendar.PixelsPerMinorScale);
...
begin
  ...
  begin
    if Dependencies[i].FinishDate > dtm then
      dtm := Dependencies[i].FinishDate;
  end;
  if dtm > Value then
  begin
...
begin
  Result :=
    (DrawRect.Left = -1) and (DrawRect.Right = -1) and (DrawRect.Top = -1) and
    (DrawRect.Bottom = -1);
end;
...
begin
  ...
  Result.Top := FDrawRect.Top + 1;// + (FDrawRect.Bottom - FDrawRect.Top) div 3;
  Result.Bottom := FDrawRect.Bottom - 1;// - (FDrawRect.Bottom - FDrawRect.Top) div 3;

  Result.Left := FDrawRect.Left + 1;
  Result.Right := Round(UnitsBetweenDates(FGantt.Calendar.VisibleStart, FIntervalDone,
    FGantt.Calendar.MinorScale, FHoursPerDay) * FGantt.Calendar.PixelsPerMinorScale);
...
begin
  ...
  Result := Rect(FDrawRect.Right - aSize, FDrawRect.Top, FDrawRect.Right,
    FDrawRect.Bottom);
end;
...
begin
  Result :=
    FVisible and ((Parent = nil) or ((Parent <> nil) and Parent.Visible));
end;
...
begin
  Result :=
    ((IntervalCount = 0) and FVisible) or ((IntervalCount > 0) and
    Interval[0].Visible);
end;
...
begin
  ...
  if DoClip then
  begin
    ClipRgn := CreateRectRgn(0, 0, Width - FVertScrollBar.Width *
      integer(FVertScrollBar.Visible), Height - FHorzScrollBar.Height *
  ...
  try
    ...
    for I := aTop to List.Count - 1 do
      if Assigned(TInterval(List[I]).FBP) then
        TInterval(List[I]).FBP(TInterval(List[I]), aCanvas,
          Rect(0, (I - aTop) * PixelsPerLine + 1 + StartDrawIntervals,
          aCanvas.Width, ((I - aTop) * PixelsPerLine) + PixelsPerLine +
          StartDrawIntervals + 1),
          VisibleStart, VisibleFinish, UnitsBetweenDates(VisibleStart,
          VisibleStart + 1, MinorScale, FHoursPerDay) * PixelsPerMinorScale);
    ...
    begin
      ...
      begin
        with aCanvas do
        begin
          if (CurrInterval.IntervalType = itAction) and
            (CurrInterval.Style = isDefault) then
          begin
            CurrInterval.Top :=
              (I - aTop) * PixelsPerLine + 1 + StartDrawIntervals +
              PixelsPerLine div 2 - PixelsPerLine div 4;
          ...
          begin
            ...
            CurrInterval.Bottom :=
              (I - aTop) * PixelsPerLine + 1 + StartDrawIntervals + 4;
            ...
            begin
              Polygon
              (
                [Point(Left, Bottom), Point(Left, Bottom + 6),
                Point(Left + 4, Bottom + 2), Point(Right - 5, Bottom + 2),
                Point(Right - 1, Bottom + 6), Point(Right - 1, Bottom)]
                );
            end;
          ...
          begin
            ...
            CurrInterval.Bottom :=
              ((I - aTop) + 1) * PixelsPerLine + StartDrawIntervals -
              IntervalHeight div 4;
            ...
            {
            {$IFDEF WINDOWS} //patrón de relleno:
            ...
            RoundRect(CurrInterval.DrawRect, 5, 5);
            Brush.Bitmap := nil;
            Brush.Color := CurrInterval.Color;
            if CurrInterval.InCriticalPath then
              Brush.Color := clRed;
            ...
            begin
              Brush.Style := bsClear;
              TextOut(CurrInterval.DrawRect.Right + 10, CurrInterval.DrawRect.Top,
                CurrInterval.Task);
            end;
          end;
        end;
...
begin
  if not FromInterval.Visible {or not FromInterval.Opened} then
    Exit;
  if not ToInterval.Visible {or not ToInterval.Opened} then
    Exit;
  ...
  begin
    ...
    Pen.Color := RGB(51,51,51);//clWhite;
    //Pen.Style := psDot;
    ...
    begin
      ...
      (
        FromRect.Right + 10,
        FromRect.Top + (ToRect.Top + (ToRect.Bottom - ToRect.Top) div
        2 - FromRect.Top + (FromRect.Bottom - FromRect.Top) div 2) div 2
        );
    ...
    begin
      ...
      begin
        ...
        (
          ...
          (UnitsBetweenDates(VisibleStart, VisibleStart + 1, MinorScale, FHoursPerDay) *
          PixelsPerMinorScale) * FromInterval.WaitTime),
          FromRect.Top + (FromRect.Bottom - FromRect.Top) div 2
          );
        Pen.Style := psDot;
        Pen.Color := clWhite;
      ...
      begin
        ...
        (
          ...
          Point(ToRect.Left + Plus, ToRect.Top),
          Point(ToRect.Left + Plus + 5, ToRect.Top - 6),
          Point(ToRect.Left + Plus, ToRect.Top - 6)
          ]
        );
...
begin
  ...
  begin
    ...
    begin
      ...
      R.Left := {Trunc}Round(UnitsBetweenDates(VisibleStart,
        FCurrentDate, MinorScale, FHoursPerDay) * PixelsPerMinorScale);
      ...
      // Draw Days off
      DrawHeight := Min((MajorScaleHeight + MinorScaleHeight + 1 +
        PixelsPerLine * nItems), Height);
      //if FDrawDaysOff then
      begin
        Brush.Color := clBtnFace;
        if isFreeDay(FCurrentDate) then
        begin
          fillRect(r.Left + 1, MajorScaleHeight + MinorScaleHeight +
            1, r.right, DrawHeight);
        end;
      end;
      ...
      // Draw Today Line:
      // TODO: mejorar la posición de la línea dependiendo de la hora
      Pen.Style := psDash;
      Pen.Width := 2;
      Pen.Color := clBlue;
      ...
      begin
        ...
        R.Left := Round(UnitsBetweenDates(VisibleStart, Now,
...
begin
  ...
  begin
    ...
    R.Left := {Trunc}Round(UnitsBetweenDates(VisibleStart, FCurrentDate,
      MinorScale, FHoursPerDay) * PixelsPerMinorScale);
    R.Right := {Trunc}Round(UnitsBetweenDates(VisibleStart,
      IncTime(FCurrentDate, MajorScale, 1), MinorScale, FHoursPerDay) *
      PixelsPerMinorScale);
...
begin
  ...
  begin
    ...
    begin
      ...
      R := Rect(0, 0, Width - FVertScrollBar.Width *
        integer(FVertScrollBar.Visible), Height - FHorzScrollBar.Height *
        integer(FHorzScrollBar.Visible));
  ...
  begin
    ...
    if ((ScrollCode = scPosition) and (FHorzScrollBar.Max -
      ...
      (FHorzScrollBar.Max = ScrollPos)) then
    begin
...
begin
  ...
  try
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            IsInRect(Message.XPos, Message.YPos, CurrInterval.LeftMoveRect) then
          begin
    ...
    begin
      ...
      if Assigned(FDragInterval.FBP) then
        FDragInterval.FBP(FDragInterval, Canvas,
          Rect(0, (aIdx - aTop) * PixelsPerLine + 1 + StartDrawIntervals,
          Width, ((aIdx - aTop) * PixelsPerLine) + PixelsPerLine +
          StartDrawIntervals + 1),
          VisibleStart, VisibleFinish, UnitsBetweenDates(VisibleStart,
          VisibleStart + 1, MinorScale, FHoursPerDay) * PixelsPerMinorScale);
      ...
      case FDragType of
        ...
        begin
          ...
          begin
            FDragRect := Rect(Message.XPos - FFromDragPoint,
              FDragRect.Top, Message.XPos - FFromDragPoint + FDragRect.Right -
              FDragRect.Left, FDragRect.Bottom);
          end;
        end;
        ...
        begin
          ...
          begin
            ...
            Canvas.Brush.Color := clBlue;
            Canvas.DrawFocusRect(
              Rect(FConnectInterval.DrawRect.Left + 1,
              FConnectInterval.DrawRect.Top + 1, FConnectInterval.DrawRect.Right -
              1, FConnectInterval.DrawRect.Bottom - 1)
              );
          end;
  ...
  begin
    ...
    if (aDiff <> 0) or (round((FConnectFromPoint.Y - Message.YPos) /
      FGantt.Tree.DefaultRowHeight) <> 0) then
    begin
  ...
  begin
    ...
    case FDragType of
      ...
      begin
        ...
        begin
          FDragInterval.NetDuration :=
            FDragInterval.NetDuration + (NewDate - FDragInterval.FinishDate);
          FDragInterval.FinishDate := NewDate;
        end;
      end;
      ditLeftMove:
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Stamp.Date > 0 then
          S := IntToStr(Stamp.Date) + 'd ';
        if Stamp.Time > 0 then
          S := S + ReturnHM(Stamp.Time);
      ...
      else if (GetColumnType(ACol) = gctStart) and (CurrInterval <> nil) then
      begin
        WriteText
        (
          Canvas,
      ...
      else if (GetColumnType(ACol) = gctFinish) and (CurrInterval <> nil) then
      begin
        WriteText
        (
          Canvas,
...
procedure TGanttTree.UpdateTreeList;
  ...
  begin
    tmp := '';
    if aTask.Parent <> nil then
      tmp := GetIndex(aTask.Parent) + '.';
