...
uses
  Classes, SysUtils, Controls, Forms, ExtCtrls, EasyDockSite, Graphics, ComCtrls,
  fFloatingSite;
...
type
  ...
  protected
    ...
    procedure pnlDockDrop(Sender: TObject; Source: TDragDockObject; X, Y: integer);
...
*)
  ...
  protected //utilities
    function ReloadForm(const AName: string; fMultiInst: boolean;
      fVisible: boolean): TWinControl; virtual;
  ...
  public
    ...
    function CreateDockable(const AName: string; fMultiInst: boolean;
      fWrap: boolean = True; fVisible: boolean = False): TWinControl;
    function MakeDockable(AForm: TWinControl; fWrap: boolean = True;
      fVisible: boolean = False): TForm; override;
    procedure DumpSites;
    //persistence
    procedure LoadFromStream(Stream: TStream);
    procedure SaveToStream(Stream: TStream);
    function ReloadDockedControl(const AName: string;
      Site: TWinControl): TControl; virtual;
...
{$ENDIF}
  ...
  begin
    if (side in AllowedSides) and (side in Sides) then
    begin
      ...
      begin
        ...
        // TTabControl
        {tbc := TTabControl.Create(AForm);
        tbc.Parent := AForm;
        tbc.Align := side;
        tbc.TabPosition := tpLeft;
        tbc.Width := tbc.TabWidth;
        tbc.Tabs.Add('javi'); }
...
function TDockMaster.CreateDockable(const AName: string; fMultiInst: boolean;
  fWrap: boolean; fVisible: boolean): TWinControl;
begin
...
{$ENDIF}
  ...
  //IDE?
  if ForIDE and fWrap and assigned(Site) and assigned(Site.DockManager) then
  begin
    Site.Invalidate;
  end;
end;
...
var
  ...
  begin
    ...
    begin
      ...
      with SiteRec do
      begin
        DebugLn('Bounds=x(%d-%d) y(%d-%d)', [Bounds.Left, Bounds.Right,
          Bounds.Top, Bounds.Bottom]);
        DebugLn('Extent=x(%d-%d) y(%d-%d)', [Extent.Left, Extent.Right,
...
procedure TDockMaster.SaveToStream(Stream: TStream);
  ...
  *)
    ...
    else if Site.HostDockSite = nil then
    begin
...
function TDockMaster.ReloadForm(const AName: string; fMultiInst: boolean;
  fVisible: boolean): TWinControl;
var
  //instname: string
  basename: string;
  fc: TWinControlClass;
...
(* Wrap a control into a floating site.
  ...
  - invisible client?
*)
  if (csDestroying in Client.ComponentState) or assigned(Client.HostDockSite)
  //already wrapped
...
{$ENDIF}
  ...
  begin
    ctl := ASite;
    s := Format('Site=%s (%d,%d)[%d,%d]', [SiteName(ctl), ctl.Left,
      ctl.Top, ctl.Width, ctl.Height]);
    while ctl <> nil do
    begin
      ...
      begin
        ...
        begin
          ...
          if assigned(Site.DockManager) then
          begin
    ...
    //clients
    Site := ASite;
    for j := 0 to Site.DockClientCount - 1 do
    begin
      ctl := site.DockClients[j];
      s := OrientString[ctl.DockOrientation];
      DebugLn('  %s.Client=%s.%s@%s (%d,%d)[%d,%d]',
        [SiteName(ASite), ctl.Owner.Name, SiteName(ctl), s, ctl.Left,
        ctl.Top, ctl.Width, ctl.Height]);
      if assigned(Site.DockManager) then
      begin
        Site.DockManager.GetControlBounds(ctl, r);
...
*)
  ...
  begin
    ...
    if (cmp is TWinControl) and wc.DockSite then
      DumpSite(wc)
    else if ctl is TControl then
    begin
      wc.DockManager.GetControlBounds(ctl, r);
      DebugLn('Client=%s in %s (%d,%d)[%d,%d] in [%d,%d]',
        [SiteName(ctl), SiteName(ctl.HostDockSite), ctl.Left, ctl.Top,
        ctl.Width, ctl.Height, r.Right, r.Bottom]);
    end;
  end;
  DebugLn('--- dump forms ---');
...
*)
  if (DockClientCount > 1) or ((Width > 1) and (Height > 1)) //NoteBook!
  then
  ...
  if not Splitter.Visible then Splitter.Show;
end;
  ...
  begin
    ...
    case Align of
      ...
      begin
        ...
        if AutoExpand then
          Adjust(0, Source.Control.TBDockHeight)
        else
          Adjust(0, -dh);
      end
      else
