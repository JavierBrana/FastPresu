...
//erase - which color(s)?
  ...
  ACanvas.Brush.Color := $009A7867;
  ACanvas.FillRect(DrawRect);

  //border
  InflateRect(DrawRect, -1, -1); //outer bevel?
  //ACanvas.Brush.Color := clBtnShadow;
  ACanvas.Brush.Color := $0082604D;
  ACanvas.Pen.Color := $009A7867;
  ...
  hsForm:
    begin
      ...
      if AOrientation = doVertical then begin
        ...
        TextOut(ACanvas.Handle, DrawRect.Left-2, DrawRect.Bottom, PChar(ACaption), Length(ACaption));
        if OldFont <> 0 then
...
//init helpers
  ...
  FSplitter.BorderStyle := bsNone; //border does NOT react!
  FSplitter.BorderWidth := 0;
