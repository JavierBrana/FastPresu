...
*)
  ...
  begin
    ...
    if not (csDestroying in ctl.ComponentState) then
      ctl.ManualDock(nil);
    DebugLn('Undocked %s P=%p H=%p', [ctl.Name, pointer(ctl.Parent),
      pointer(ctl.HostDockSite)]);
  end;
  inherited Destroy;
end;
...
procedure TEasyDockBook.FormClose(Sender: TObject; var CloseAction: TCloseAction);
var
  i: integer;
  ctl: TControl;
...
{$ENDIF}
  if Tabs.ButtonCount > 0 then
  begin
    //tab moved?
    if CurTab = nil then
    begin //current button removed
...
begin
  for i := 0 to Tabs.ButtonCount - 1 do
  begin
    btn := Tabs.Buttons[i];
...
*)
  ...
  if (ssLeft in Shift) and not DragManager.IsDragging then
  begin
    if Control <> nil then
