...
procedure TDockingSite.pnlLeftDockDrop(Sender: TObject; Source: TDragDockObject;
  X, Y: integer);
var
  w: integer;
  r: TRect;
...
*)
  if (TWinControl(Source.DragTarget).DockClientCount > 1) or
    ((Site.Width > 1) and (Site.Height > 1)) //NoteBook!
  then
...
*)
  ...
  with Source do
  begin
    if DragTarget.Align in [alLeft, alRight] then
    begin
      w := self.Width div 3;
      if DragTarget.Width < w then
      begin
        //enlarge docksite
        DisableAlign; //form(?)
        DragTarget.Width := w;
...
procedure TDockingSite.pnlLeftDockOver(Sender: TObject; Source: TDragDockObject;
  X, Y: integer; State: TDragState; var Accept: boolean);
var
  r: TRect;
...
*)
  ...
  {$IFDEF ExpandFlag}
    ...
    case Site.Align of
      alLeft: if AutoExpand then
          Adjust(-dw, 0)
        else
          Adjust(dw, 0);
      alRight: if AutoExpand then
          Adjust(dw, 0)
        else
          Adjust(-dw, 0);
      alBottom: if AutoExpand then
          Adjust(0, dh)
        else
          Adjust(0, -dh);
      else
...
procedure TDockingSite.pnlLeftGetSiteInfo(Sender: TObject; DockClient: TControl;
  var InfluenceRect: TRect; MousePos: TPoint; var CanDock: boolean);
begin
(* Signal acceptance.
...
*)
  if Site.DockClientCount <= 1 then
  begin
